...
class Client:
    ...
    def server_authentication(self,public_key):
        ...
        * session Key -> bytes
        ...
        Na = uuid.uuid4()
        print("Na ",Na.bytes)
        initial_conn_string = rsa_key.encrypt(Na.bytes+"bobby".encode())
    ...
    def send_command(self,command,seqA,seqB,encryption_key,integrity_key,path=""):
        ...
        msg = self.get_encrypted_msg_with_integrity(command_chunk,"bobby".encode()+encryption_key+self.int_to_bytes(seqA,32),integrity_key+self.int_to_bytes(seqA,32))
        self.socket.send(msg)
        if command == "Ok":
            return
    ...
    def send_data(self,data,seqA, seqB, encryption_key, integrity_key):
        ...
        chunk_size = 30
        key_string = "bobby".encode()+encryption_key
        offset = 0
        end_loop = False
...
if __name__ == '__main__':
    ...
    f_pubk = open("C:\\Users\\vismay\\Desktop\\sfta\\public_key.pem", 'rb')
